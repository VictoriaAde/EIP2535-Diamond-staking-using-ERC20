...
contract StakingFacet {
    ...
    function rewardPerSec() internal pure returns (uint256) {
        uint amount = LibAppStorage
            .REWARD_PER_SEC
            .mul(LibAppStorage.REWARD_RATE)
            .div(LibAppStorage.RATE_TOTAL_PRECISION)
            .div(12);
        return amount;
    }
    ...
    function updatePool() internal {
        ...
        if (block.timestamp > l.lastStakedTime) {
            ...
            if (reward_tot_supply > 0 && l.totalAllocatedPoints > 0) {
                ...
                uint256 calculateReward = multiplier
                    .mul(rewardPerSec())
                    .mul(s.allocatedPoints)
                    .div(l.totalAllocatedPoints);

                uint256 accRewardPerShare = l.accRewardPerShare.add(
                    (calculateReward.mul(LibAppStorage.ACC_REWARD_PRECISION))
                        .div(reward_tot_supply)
                );
            }
